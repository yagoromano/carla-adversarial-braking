...
class YOLONode(CompatibleNode):
    def __init__(self):
        ...
        self.angle_publisher = self.new_publisher(
            Float32, f"/carla/{self.role_name}/yolo/angle",
            qos_profile=QoSProfile(depth=10)
        )
        self.bbox_publisher = self.create_publisher(Float32MultiArray, '/carla/ego_vehicle/yolo/bbox', 10)
    ...
    def on_view_image(self, image: Image, view: str):
        ...
        #self.loginfo(f"Start Time (ns): {start_time.nanoseconds}")
        ...
        try:
            # Initialize angle to None
            angle = None

            results = self.yolo_model.predict(tensor, conf=0.4, device=self.device, half=True, stream=True, classes=[11])
            ...
            for result in results:
                if result.boxes is not None and len(result.boxes.xyxy) > 0:
                    ...
                    for (box, cls, conf) in zip(result.boxes.xyxy, result.boxes.cls, result.boxes.conf):
                        ...
                        # Only calculate the angle if the view is 'front'
                        if view == 'front' and class_name == 'stop sign':
                            # Calculate the angle of the detection
                            x_center = (bbox[0] + bbox[2]) / 2
                            angle = ((x_center - W / 2) / (W / 2)) * (FOV / 2)

                            # Publish the angle only if it's not None
                            if angle is not None:
                                self.angle_publisher.publish(Float32(data=angle))
                                self.publish_bbox_to_lidar(bbox)
                        ...
                        if view == 'front' and self.handle_detection(class_name, angle, bbox, confidence, view):
                            self.stop_sign_detected = True
                            
                        if view == 'front_right' and self.handle_right_detection(class_name, bbox, view) and self.stop_sign_detected: 
            ...
            # Publish detection results for stop sign
            ...
            #self.loginfo(f"End Time (ns): {end_time.nanoseconds}")
            
            total_latency = (end_time - start_time).nanoseconds / 1e9  # Total time in seconds from pulling frame to display
            #self.loginfo(f"Total Latency (Frame to Display): {total_latency:.9f} seconds")


        except Exception as e:
            self.logwarn(f"YOLO error: {e}")

    def publish_bbox_to_lidar(self, bbox):
    ...
    def handle_detection(self, class_name, angle, bbox, confidence, view):
        if class_name == 'stop sign' and 0 <= angle <= 30 and view == 'front':
            ...
            #self.loginfo(f"Distance: {self.stop_sign_distance:.2f} meters")
            return True
        else: 
            return False
    ...
    def estimate_distance(self, bbox_height):
        ...
        #self.loginfo(f"Distance: {distance:.2f} meters")

        if not self.first_detection:
            #self.loginfo(f"First stop sign detected! Distance: {distance:.2f} meters")
