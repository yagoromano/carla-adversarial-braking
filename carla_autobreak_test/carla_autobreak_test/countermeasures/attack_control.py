...
class VehicleControlNode(CompatibleNode):
    def __init__(self):
        ...
        self.target_speed_kmh = 90.0
        self.pid = PID(0.8, 0.05, 0.1, setpoint=self.target_speed_kmh)
        self.pid.output_limits = (0, 1)

        # Initialize CSV file with a specific path
        self.csv_file_path = '/home/yromanoma42@tntech.edu/carla-ros-bridge/src/carla_autobreak_test/carla_autobreak_test/stop_sign_data_normal.csv'
        print(f"CSV file will be saved to: {self.csv_file_path}")
        with open(self.csv_file_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Simulation Time', 'Velocity (km/h)', 'Distance to Stop Sign (m)'])

        # Initialize the last recorded time and the recording interval (in seconds)
        self.last_recorded_time = time.time()
        self.recording_interval = 0.5
    ...
    def publish_vehicle_control(self):
        ...
        #    control_msg.brake = 1.0  
            self.recording_interval = 1.5
            self.record_data(self.vehicle_speed, self.right_stop_sign_distance)
            ...
            self.record_data(self.vehicle_speed, self.stop_sign_distance)
    ...
    def record_data(self, velocity, distance):
        current_time = time.time()
        if current_time - self.last_recorded_time >= self.recording_interval:
            print(f"Recording data: Time={self.simulation_time}, Velocity={velocity}, Distance={distance}")
            with open(self.csv_file_path, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([self.simulation_time, velocity, distance])
